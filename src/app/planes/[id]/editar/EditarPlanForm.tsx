...
export default function EditarPlanForm({
  planId,
  lot,
  seasonName,
  initialTargetTch,
  ...
  planId: number;
  lot: { id: number; displayName: string; areaHa: number };
  seasonName: string;
  initialTargetTch: number;
  initialNotes: string;
  initialApps: AppState[];
  formulas: Formula[];
}) {
  const [targetTch, setTargetTch] = useState(String(initialTargetTch));
  const [notes, setNotes] = useState(initialNotes);
  // Ensure we always have 2 app slots
  const [apps, setApps] = useState<AppState[]>(() => {
    const padded = [...initialApps];
    while (padded.length < 2) padded.push({ formulaId: "", sacksPerHa: "", plannedDate: "" });
    return padded;
  });
  ...
  const calculations = useMemo(() => {
    ...
    const appResults: (NutrientResult | null)[] = apps.map((app) => {
      ...
      return calculateApplicationNutrients(formula, sacks, lot.areaHa);
    });
    ...
    const balance = evaluateNutrientBalance(total, tch, lot.areaHa);
    return { appResults, total, balance };
  }, [targetTch, apps, formulas, lot.areaHa]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validApps.length === 0) {
      setError("Se requiere al menos una aplicacion");
      setSubmitting(false);
      return;
    }
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-outline-variant)]/20">
                  <th className="px-3 py-2 text-left text-[10px] font-semibold uppercase tracking-wider text-[var(--color-on-surface-variant)]">Nutriente</th>
                  <th className="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-[var(--color-on-surface-variant)]">Total (kg)</th>
                  <th className="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-[var(--color-on-surface-variant)]">kg / TCH</th>
                  <th className="px-3 py-2 text-right text-[10px] font-semibold uppercase tracking-wider text-[var(--color-on-surface-variant)]">Rango</th>
                ...
                {(Object.keys(NUTRIENT_LABELS) as NutrientKey[]).map((key) => {
                  ...
                  return (
                    <tr key={key}>
                      ...
                      <td className="px-3 py-2 text-right tabular-nums">{totalKg.toFixed(1)}</td>
                      <td className="px-3 py-2 text-right tabular-nums font-medium">{b.kgPerTch.toFixed(2)}</td>
                      <td className="px-3 py-2 text-right tabular-nums text-[var(--color-on-surface-variant)]">{target.min} - {target.max}</td>
